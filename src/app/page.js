...
export default function Home() {
  ...
  const [articleInfo3, setArticleInfo3] = useState({
    articles: [],
    summaries: [],
  });
  ...
  const handleOutput = async (prompt) => {
    ...
    const getDescription = async () => {
      ...
      while (true) {
        ...
        if (done) {
          const postDescriptionText = document.getElementById(
            "descriptionHtml" + counter
          ).innerHTML;
          if (counter == 1) {
          ...
          for (let i = 0; i < keywordArr.length; i++) {
            ...
            let result = rawDescriptionText
              .toLocaleUpperCase()
              .indexOf(currentKeyword.toLocaleUpperCase());
            if (result >= 0) {
              let substring = rawDescriptionText.substring(
                result,
                result + currentKeyword.length
              );
              let pre = rawDescriptionText.substring(0, result);
              let post = rawDescriptionText.substring(
                result + currentKeyword.length
              );
              rawDescriptionText =
                pre + `<a class='keywords' >` + substring + "</a>" + post;
            } //style={{text-decoration: 'underline !important', display: 'inline !important'}} onHover={style={{color: 'purple !important'}}}
          }

          document.getElementById("descriptionHtml" + counter).innerHTML =
            rawDescriptionText;
    ...
    const getVideos = async () => {
      ...
      const videoInfo = {
        videos: [
          parsedResponse.video1,
          parsedResponse.video2,
          parsedResponse.video3,
        ],
        summaries: [
          parsedResponse.summary1,
          parsedResponse.summary2,
          parsedResponse.summary3,
        ],
      };
      if (counter == 1) {
    ...
    const getImages = async () => {
      ...
      const images = [
        parsedResponse.image1,
        parsedResponse.image2,
        parsedResponse.image3,
      ];
      if (counter == 1) {
        setImages(images);
      } else if (counter == 2) {
    ...
    const getArticles = async () => {
      ...
      const articleInfo = {
        articles: [
          parsedResponse.article1,
          parsedResponse.article2,
          parsedResponse.article3,
        ],
  ...
  <h3 className="">Subject</h3>
    ...
    {questions.map((question, index) => (
      ...
      <div className="App">
        ...
        <div className="absolute left-3 top-3 w-32 h-32">
          <img src="bloom.png"></img>
        </div>
        <div className="relative">
          <div className="mx-auto max-w-7xl sm:px-6 lg:px-8 relative">
            <div className="relative isolate overflow-hidden px-6 py-12 text-center sm:px-16 dark:bg-transparent">
              <p className="relative pt-36 mx-auto max-w-2xl text-4xl font-bold tracking-tight text-dark-gray dark:text-dark-gray sm:text-4xl">
                let learning{" "}
                <span className="relative font-semibold bg-gradient-to-r from-blue via-blue to-purple inline-block text-transparent bg-clip-text">
                  bloom.
                </span>
              </p>
              <form onSubmit={handleChatSubmit}>
                <label
                  className="mx-auto mt-8 relative bg-white min-w-sm max-w-2xl flex flex-col md:flex-row items-center justify-center border py-2 px-2 rounded-2xl gap-2 shadow-lg focus-within:border-gray-300"
                  htmlFor="search-bar"
                >
                  <input
                    id="search-bar"
                    ...
                    name="prompt"
                    className="relative px-6 py-2 w-full rounded-md flex-1 outline-none bg-white"
                    required=""
                  />
                  <button
                    type="submit"
        ...
        <div
          id="component1"
          className="ml-24 mr-24 mb-4 rounded-2xl z-10 hidden"
        >
          <div className="collapse rounded-2xl">
            ...
            <div className="collapse-content bg-white text-dark-gray border-gray-md border-t-0 border-r-2 border-l-2 border-b-2 rounded-2xl peer-checked:rounded-tr-none peer-checked:rounded-tl-none">
              ...
              <pre
                className="flex font-sans w-9/12 whitespace-pre-line"
                id="descriptionHtml1"
              >
                {description}
              </pre>
                  ...
                  <div key={image.thumbnail}>
                    <div className="h-full w-72 flex flex-row justify-center space-x-20 items-center mb-12">
                      <a target="_blank" href={image.original.link}>
                        <img
                          src={image.thumbnail}
                          className="object-contain object-scale-down mt-4 m-auto"
                        />
                      </a>
                    </div>
                  </div>
                    ...
                    <div className="flex">
                      <div className="h-full w-72">
                        ...
                        <a
                          target="_blank"
                          href={video.link}
                          className="flex justify-center mt-2 underline decoration-gray-md font-medium hover:font-semibold"
                        >
                          {video.title}
                        </a>
                        <a className="flex justify-center mt-2">
                          ...
                          <dialog id={index} className="modal">
                            <div className="modal-box bg-white">
                              <h3 className="font-bold text-lg  decoration-gray-md">
                                {video.title}
                              </h3>
                              <hr className="h-px bg-gray-md border-0"></hr>

                              <p className="py-4">
                                {videoInfo.summaries[index].text}
                              </p>
                            </div>
                    ...
                    <li>
                      <details className="group">
                        <summary className="flex items-center gap-3 px-4 py-3 font-medium marker:content-none hover:cursor-pointer">
                          ...
                          <span className="question hover:font-semibold">
                            {question.question}
                          </span>
                        </summary>
              ...
              <div className="flex flex-col space-y-2">
                {articleInfo.articles.map((article, index) => (
                  <div key={article.link}>
                    <a target="_blank" href={article.link}>
                      <p className="underline decoration-gray-md font-medium hover:font-semibold">
    ...
    return(
        ...
        <div className="">
          <footer className="flex justify-end mr-2 w-full fixed bottom-0">
            ...
            <div className="">
              <span className="text-neutral-400 text-md font-normal bg-white">
                made with{" "}
              </span>
              <span className="text-rose-500 text-md font-normal">❤ </span>
              <span className="text-neutral-400 text-md font-normal">
                from NJ
              </span>
              <span className="text-rose-500 text-md font-normal font-['Inter']">
                {" "}
                ️
              </span>
            </div>
